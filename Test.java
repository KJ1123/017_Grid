...
public class Test {
	...
	public static <E> int paths(Grid<E> grid, Coordinate start, Coordinate end) {
		...
		if (start.equals(end)) {
			...
			return paths(grid, new Coordinate(x1, y1 + 1), end);
		}
		else if (y1 == y2) {
			return paths(grid, new Coordinate(x1 + 1, y1), end);
		}
		else {
			return paths(grid, new Coordinate(x1 + 1, y1), end) + paths(grid, new Coordinate(x1, y1 + 1), end);
		}
	...
	public static <E> String onePath(Grid<E> grid, Coordinate start, Coordinate end) {
		...
		if (start.equals(end)) {
			...
			//path += grid.get(x1,y1 + 1);
			return "-" + grid.get(x1,y1+1) + onePath(grid, new Coordinate(x1, y1 + 1), end);
		}
		else {
			//path += grid.get(x1+1,y1);
			return "-" + grid.get(x1,y1+1) + onePath(grid, new Coordinate(x1 + 1, y1), end);
		}
